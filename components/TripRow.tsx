...
import { VEHICLE_ICON, STATUS_CONFIG, CYCLE_CONFIG } from "@/interfaces/trip.interface";
...
const TripRow: React.FC<Props> = ({ trip, onEdit, onDelete, onRowClick }) => {
    ...
    return (
        <div
            className={`grid items-center px-4 py-3.5 border-b cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 ${isDark ? "border-gray-800" : "border-gray-100"
                }`}
        ...
        >
            ...
            <div className="flex items-center gap-2 min-w-0">
                <div
                    className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-black border ${isDark
            ...
            {/* Vehicle */}
            <div className={`text-xs flex items-center gap-1.5 ${isDark ? "text-gray-300" : "text-gray-700"}`}>
            ...
            {/* Cycle */}
            <div>
                <span className={`text-[10px] font-bold px-2 py-1 rounded-lg ${cycle.badge}`}>
                    {cycle.label}
                </span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
                ...
                <span className={`text-[10px] font-bold px-2 py-1 rounded-lg ${status.badge}`}>
                    {status.label}
                </span>
            </div>
            ...
            <div className="flex gap-1 flex-wrap">
                {notifFlags.map((f) => (
                    <span
                        key={f.key}
                        className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${f.active
                            ? isDark
            ...
            <div className="flex gap-1.5" onClick={(e) => e.stopPropagation()}>
                <button
                    onClick={() => onEdit(trip)}
                    className={`w-7 h-7 flex items-center justify-center rounded-lg border transition-colors ${isDark
