...
export default function HistoryTracking() {
    ...
    useEffect(() => {
        ...
        const previousIndex = Math.min(previousIndexRef.current, historyData.length - 1);
        ...
        const startPoint = historyData[Math.min(startIndex, historyData.length - 1)];
        const startPosition = {
        ...
        const animate = (timestamp: number) => {
            ...
            setAnimatedPosition({
                lat: startPosition.lat + (targetPosition.lat - startPosition.lat) * progress,
                lng: startPosition.lng + (targetPosition.lng - startPosition.lng) * progress,
            });
                                ...
                                <div className="space-y-3">
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div
                                                className={`text-lg font-black ${currentPoint.ignition
                                                        ? "text-emerald-500"
                                                        : "text-slate-400"
                                                    }`}
                                            >
                                            ...
                                            <div className="flex items-center gap-2">
                                                {[1, 2, 4].map((speed) => (
                                                    <button
                                                        ...
                                                        className={`h-8 rounded-lg px-3 text-xs font-bold transition ${playbackSpeed === speed
                                        ...
                                        <div className="flex items-center gap-2">
                                            {[1, 2, 4].map((speed) => (
                                                <button
                                                    key={speed}
                                                    onClick={() => setPlaybackSpeed(speed)}
                                                    className={`h-8 rounded-lg px-2.5 text-xs font-bold ${playbackSpeed === speed
                                                            ? "bg-slate-900 text-white"
