...
const CreateUser: React.FC = () => {
  ...
  const [additionalPermissions, setAdditionalPermissions] =
    useState<AdditionalPermission[]>([]);
  ...
  const isPermissionFromRole = (formId: number) => {
    return rolePermissions.some(p => p.formId === formId &&
  ...
  const handleSubmit = async () => {
    if (!formData.accountName || !formData.accountCode || !formData.emailAddress) {
      ...
      <div className={`${isDark ? "dark" : ""} mt-16 sm:mt-20`}>
        <div className={`min-h-screen ${isDark ? "bg-background" : ""} p-3 sm:p-4 md:p-6 flex items-center justify-center`}>
          <div className="text-center">
            ...
            <p className={isDark ? "text-gray-400" : "text-gray-600"}>Loading user data...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className={`min-h-screen ${isDark ? "bg-background" : ""} p-3 sm:p-4 md:p-6`}>
        <div className="mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
            <div>
              <h1 className={`text-xl sm:text-2xl md:text-3xl font-bold ${isDark ? "text-foreground" : "text-gray-900"}`}>
            ...
            <div className="flex gap-2 sm:gap-3">
              <button
                ...
                className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 text-sm sm:text-base rounded-lg font-medium transition-colors ${isDark ? "bg-gray-800 text-gray-300 hover:bg-gray-700 border border-gray-700" : "bg-white text-gray-700 hover:bg-gray-50 border border-gray-300"
                  }`}
              >
                Cancel
              </button>
        ...
        <div className={`border-b ${isDark ? "border-gray-700" : "border-gray-200"} mb-4 sm:mb-6`}>
          <div className="flex gap-1 sm:gap-2 overflow-x-auto scrollbar-hide">
            {tabs.map((tab) => (
              <button
                ...
                style={{
                  borderBottomColor: activeTab === tab.id ? selectedColor : "transparent",
                  color: activeTab === tab.id ? selectedColor : isDark ? "#9CA3AF" : "#6B7280",
                }}
              >
                {tab.label}
              </button>
        ...
        <div>
            ...
            <Card isDark={isDark}>
              <div className="p-4 sm:p-6">
                <div className="flex items-center gap-2 mb-1">
                  <User className={`w-4 h-4 sm:w-5 sm:h-5`} style={{ color: selectedColor }} />
                  <h2 className={`text-lg sm:text-xl font-bold ${isDark ? "text-foreground" : "text-gray-900"}`}>
                    Personal Information
                  </h2>
                </div>
                <p className={`text-xs sm:text-sm mb-4 sm:mb-6 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                  Basic identity details used for login and display.
                </p>

                <div className="mb-6">
                  <label className={`block text-xs sm:text-sm font-medium mb-3 ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                    Profile Picture
                  </label>
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full flex items-center justify-center overflow-hidden ${isDark ? "bg-gray-800 border-2 border-gray-700" : "bg-gray-100 border-2 border-gray-300"
                        }`}>
                        {avatarPreview ? (
                          <img src={avatarPreview} alt="Avatar preview" className="w-full h-full object-cover" />
                        ) : (
                          <User className={`w-10 h-10 sm:w-12 sm:h-12 ${isDark ? "text-gray-600" : "text-gray-400"}`} />
                        )}
                      </div>
                      {avatarPreview && (
                        <button
                    ...
                    <div className="flex-1">
                      <label
                        ...
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg font-medium cursor-pointer transition-colors ${isDark ? "bg-gray-800 text-gray-300 hover:bg-gray-700 border border-gray-700" : "bg-white text-gray-700 hover:bg-gray-50 border border-gray-300"
                          }`}
                      >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Jane"
                      className={`w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base rounded-lg border transition-colors ${isDark ? "bg-gray-800 border-gray-700 text-foreground placeholder-gray-500" : "bg-white border-gray-300 text-gray-900 placeholder-gray-400"
          ...
          {activeTab === "access" && (
            <Card isDark={isDark}>
              <div className="p-4 sm:p-6">
                <div className="flex items-center gap-2 mb-1">
                  <Building2 className={`w-4 h-4 sm:w-5 sm:h-5`} style={{ color: selectedColor }} />
                  <h2 className={`text-lg sm:text-xl font-bold ${isDark ? "text-foreground" : "text-gray-900"}`}>
                    Access & Roles
                  </h2>
                </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <label className={`block text-xs sm:text-sm font-medium mb-1.5 sm:mb-2 ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                      Account Association <span className="text-red-500">*</span>
                    </label>
                    <select
                      name="accountId"
                      value={formData.accountId}
                      onChange={handleInputChange}
                      className={`w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base rounded-lg border transition-colors ${isDark ? "bg-gray-800 border-gray-700 text-foreground" : "bg-white border-gray-300 text-gray-900"
                        } focus:outline-none focus:ring-2 focus:ring-purple-500/20`}
                    >
                  ...
                  {formData.roleId > 0 && (
                    <div className="mt-6">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className={`text-base font-semibold ${isDark ? "text-foreground" : "text-gray-900"}`}>
                          Permission Matrix
                        </h3>
                        {loadingPermissions && (
                      ...
                      <p className={`text-xs mb-4 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                        Role permissions are shown below. You can grant additional permissions specific to this user.
                      </p>
                      ...
                      {rolePermissions.length > 0 ? (
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className={`border-b ${isDark ? "border-gray-700" : "border-gray-200"}`}>
                                <th className={`text-left py-3 px-4 font-semibold text-sm ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                                  RESOURCE
                                </th>
                                <th className={`text-center py-3 px-4 font-semibold text-sm ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                                  READ
                                </th>
                                <th className={`text-center py-3 px-4 font-semibold text-sm ${isDark ? "text-gray-300" : "text-gray-700"}`}>
                                  WRITE
                            ...
                            <tbody>
                              {rolePermissions.map((permission) => {
                                const isFromRole = isPermissionFromRole(permission.formId);
                                const isAdditional = isAdditionalPermission(permission.formId);
                                ...
                                return (
                                  ...
                                  >
                                    <td className={`py-4 px-4 ${isDark ? "text-foreground" : "text-gray-900"}`}>
                                      <div className="font-medium">{permission.formName}</div>
                                    ...
                                    <td className="text-center py-4 px-4">
                                      <input
                                        ...
                                        disabled={isFromRole}
                                        onChange={() => toggleAdditionalPermission(permission.formId)}
                        ...
                        <div className={`text-center py-8 rounded-lg border ${isDark ? "border-gray-700 bg-gray-800" : "border-gray-300 bg-gray-50"}`}>
                          <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                            {loadingPermissions ? "Loading permissions..." : "No permissions available for this role"}
                          </p>
                        </div>
                      )}

                      <div className={`mt-4 p-3 rounded-lg ${isDark ? "bg-gray-800" : "bg-blue-50"}`}>
                        <p className={`text-xs ${isDark ? "text-gray-400" : "text-gray-700"}`}>
                          <strong>Note:</strong> Role permissions (Read, Write, Delete, Export) are inherited from the selected role and cannot be modified.
                          Use the "Additional" column to grant extra permissions specific to this user beyond their role.
                        </p>
                      </div>
                    </div>
                  )}
          ...
          {activeTab === "security" && (
            <Card isDark={isDark}>
              <div className="p-4 sm:p-6">
                <div className="flex items-center gap-2 mb-1">
                  <Shield className={`w-4 h-4 sm:w-5 sm:h-5`} style={{ color: selectedColor }} />
                  <h2 className={`text-lg sm:text-xl font-bold ${isDark ? "text-foreground" : "text-gray-900"}`}>
                    Security Settings
                  </h2>
                </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                  <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-3 sm:py-4 border-b ${isDark ? "border-gray-700" : "border-gray-200"}`}>
                    <div className="flex-1">
                      <h3 className={`text-sm sm:text-base font-medium mb-1 ${isDark ? "text-foreground" : "text-gray-900"}`}>
                        Account Status
                      </h3>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <div
                          ...
                          style={{ backgroundColor: userStatus ? selectedColor : isDark ? "#374151" : "#D1D5DB" }}
                        ></div>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-3 sm:py-4 border-b ${isDark ? "border-gray-700" : "border-gray-200"}`}>
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      <div
                        className="w-11 h-6 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"
                        style={{ backgroundColor: twoFactorEnabled ? selectedColor : isDark ? "#374151" : "#D1D5DB" }}
                      ></div>
                    </label>
                  </div>
                    ...
                    <div className="py-3 sm:py-4">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <button
                          ...
                          className={`w-full sm:w-auto px-3 sm:px-4 py-2 text-sm sm:text-base rounded-lg font-medium transition-colors ${isDark ? "bg-gray-800 text-gray-300 hover:bg-gray-700 border border-gray-700" : "bg-white text-gray-700 hover:bg-gray-50 border border-gray-300"
                            }`}
                        >
                          Send Link
                        </button>
