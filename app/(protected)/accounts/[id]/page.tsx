...
const EditAccount: React.FC = () => {
  ...
  useEffect(() => {
    if (formData.countryId && !formData.businessTimeZone && countries.length > 0) {
      const selectedCountry = countries.find(c => c.countryId === Number(formData.countryId));
      ...
      <div className={`${isDark ? "dark" : ""} `}>
        <div className={`min-h-screen ${isDark ? "bg-background" : "bg-gray-50"} p-6 flex items-center justify-center`}>
          <div className="text-center">
            ...
            <p className={isDark ? "text-gray-400" : "text-gray-600"}>Loading account details...</p>
          </div>
        </div>
      </div>
    );
